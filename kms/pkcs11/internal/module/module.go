...
// Handle is a shared handle pointing at a PKCS#11 context.
type module struct {
	*pkcs11.Ctx
...
// Open returns a module handle, either by loading a new dynamic library from
// path or by reusing one from a global cache. Modules are always initialized.
...
func (h *module) GetToken(selectors ...TokenSelector) (*Token, error) {
	if len(selectors) == 0 {
		return nil, errors.New("need at least one selector")
	}

	list, err := h.GetSlotList(true)
	if err != nil {
		return nil, fmt.Errorf("failed to pkcs#11 GetSlotList: %w", err)
	}

	for _, slot := range list {
		info, err := h.GetTokenInfo(slot)
		if err != nil {
